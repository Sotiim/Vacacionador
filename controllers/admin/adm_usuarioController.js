...
controller.list = (req, res) => {
    ...
    req.getConnection(async (err, conn) => {
        await conn.query('SELECT flota.Flota, puesto.Puesto, rango.Rango,usuario.numTrabajador,usuario.Nombre,usuario.apePaterno,usuario.apeMaterno,usuario.estado,usuario.email FROM (((usuario INNER JOIN flota ON usuario.idFlota = flota.idFlota) INNER JOIN rango ON usuario.idRan = rango.idRan) INNER JOIN puesto ON usuario.idPues = puesto.idPues) Where estado= "Pendiente" or estado= "Rechazado";', (err, usuarios) => {
            if (err) {
                res.json(err);
            }
...
/**
* Desarrollador: Cesar Alberto Garcia Aranda
*   Parametros: req, almacena la conexión con la base de datos.
*               res, respuesta al ejecutar una sentencia query.   
